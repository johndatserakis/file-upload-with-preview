const y = function () {
  ...
  for (const e of document.querySelectorAll('link[rel="modulepreload"]')) E(e);
  new MutationObserver((e) => {
    for (const B of e)
      if (B.type === 'childList')
        for (const I of B.addedNodes) I.tagName === 'LINK' && I.rel === 'modulepreload' && E(I);
  }).observe(document, { childList: !0, subtree: !0 });
  function g(e) {
    const B = {};
    return (
      e.integrity && (B.integrity = e.integrity),
      e.referrerpolicy && (B.referrerPolicy = e.referrerpolicy),
...
Element.prototype.matches ||
  (Element.prototype.matches =
    ...
    function (C) {
      for (
        var A = (this.document || this.ownerDocument).querySelectorAll(C), g = A.length;
        --g >= 0 && A.item(g) !== this;
...
Array.prototype.findIndex ||
  Object.defineProperty(Array.prototype, 'findIndex', {
    value: function (C) {
      ...
      for (var E = arguments[1], e = 0; e < g; ) {
        var B = A[e];
        if (C.call(E, B, e, A)) return e;
        e++;
      }
      return -1;
    },
...
(function () {
  ...
  function C(A, g) {
    g = g || { bubbles: !1, cancelable: !1, detail: null };
    var E = document.createEvent('CustomEvent');
    return E.initCustomEvent(A, g.bubbles, g.cancelable, g.detail), E;
  }
...
class k {
  constructor(A, g = { text: {}, images: {} }) {
    ...
    this.uploadId = A;
    const { images: E, maxFileCount: e, presetFiles: B, showDeleteButtonOnImages: I, text: Q } = g;
    (this.options.showDeleteButtonOnImages = I != null ? I : !0),
      (this.options.maxFileCount = e != null ? e : 0),
      (this.cachedFileArray = []),
      (this.currentFileCount = 0),
      (this.options.presetFiles = B != null ? B : []),
      this.options.presetFiles &&
        this.addImagesFromPath(this.options.presetFiles).catch((R) => {
          console.warn(`${R.toString()}`),
            console.warn('An image you added from a path cannot be added to the cachedFileArray.');
        });
    const { browse: t, chooseFile: o, selectedCount: s } = Q;
    (this.options.text.chooseFile = o != null ? o : i),
      (this.options.text.browse = t != null ? t : p),
      (this.options.text.selectedCount = s != null ? s : b);
    const d = document.querySelector(`.custom-file-container[data-upload-id="${this.uploadId}"]`);
    if (!d)
      throw new Error(`Could not find a 'custom-file-container' with the id of: ${this.uploadId}`);
    this.el = d;
    const n = this.el.querySelector('input[type="file"]'),
      m = this.el.querySelector('.custom-file-container__custom-file__custom-file-control'),
      h = this.el.querySelector('.custom-file-container__image-preview'),
      S = this.el.querySelector('.custom-file-container__image-clear');
    if (n && m && h && S)
      console.log('input', n),
        (this.input = n),
        (this.inputLabel = m),
    ...
    (this.options.images.baseImage = r != null ? r : F),
      (this.options.images.successPdfImage = c != null ? c : M),
      (this.options.images.successVideoImage = u != null ? u : P),
      (this.options.images.successFileAltImage = l != null ? l : L),
      (this.options.images.backgroundImage = a != null ? a : D),
  ...
  bindClickEvents() {
    this.input.addEventListener(
      ...
      this.clearButton.addEventListener(
        'click',
        (A) => {
          const g = new CustomEvent('fileUploadWithPreview:clearButtonClicked', {
            detail: { uploadId: A.target.uploadId },
          });
          window.dispatchEvent(g), this.clearPreviewPanel();
        },
        !0,
      ),
      this.imagePreview.addEventListener('click', (A) => {
        ...
        if (
          !!g &&
          g.matches('.custom-file-container__image-multi-preview__single-image-clear__icon')
        ) {
          const E = g.getAttribute('data-upload-token'),
            e = this.cachedFileArray.findIndex((B) => B.token === E);
          this.deleteFileAtIndex(e);
        }
      });
  }
  addFiles(A) {
    ...
    this.options.maxFileCount > 0 &&
      A.length + this.cachedFileArray.length > this.options.maxFileCount &&
      (g = this.options.maxFileCount - this.cachedFileArray.length),
      this.input && this.input.multiple
        ? (this.currentFileCount += g)
        : ((this.currentFileCount = g), (this.cachedFileArray = [])),
      Array.from(A).forEach((e) => {
        (e.token =
          Math.random().toString(36).substring(2, 15) +
          Math.random().toString(36).substring(2, 15)),
          this.cachedFileArray.push(e),
          this.renderFile(e);
      });
    const E = new CustomEvent('fileUploadWithPreview:imagesAdded', {
      detail: {
        files: A,
  ...
  renderFile(A) {
    this.currentFileCount === 0
      ? (this.inputLabel.innerHTML = this.options.text.chooseFile || i)
      : this.currentFileCount === 1
      ? (this.inputLabel.textContent = A.name)
      : (this.inputLabel.innerHTML = `${this.currentFileCount} ${this.options.text.selectedCount}`),
      this.addBrowseButton(this.options.text.browse || p),
      ...
      (g.onload = () => {
        if (!this.input.multiple) {
          A.type.match('image/png') || A.type.match('image/jpeg') || A.type.match('image/gif')
            ? (this.imagePreview.style.backgroundImage = `url("${g.result}")`)
          ...
          A.type.match('image/png') || A.type.match('image/jpeg') || A.type.match('image/gif')
            ? this.options.showDeleteButtonOnImages
              ...
              <div
                ...
                style="background-image: url('${g.result}'); "
              >
  ...
  addImagesFromPath(A) {
    return new Promise(async (g, E) => {
      const e = [];
      A.forEach(async (B) => {
        let I, Q;
        try {
          (I = await fetch(B, { mode: 'cors' })), (Q = await I.blob());
        } catch (t) {
          E(t);
        }
        if (Q) {
          const t = new Blob([Q], { type: Q.type });
          (t.name = B.split('/').pop()), e.push(t);
        }
      }),
        this.addFiles(e),
        g();
  ...
  deleteFileAtIndex(A) {
    ...
    const g = new CustomEvent('fileUploadWithPreview:imageDeleted', {
      detail: {
        index: A,
        uploadId: this.uploadId,
        cachedFileArray: this.cachedFileArray,
  ...
  refreshPreviewPanel() {
    (this.imagePreview.innerHTML = ''),
      ...
      this.cachedFileArray.forEach((A) => this.renderFile(A)),
  ...
  clearPreviewPanel() {
    this.addBrowseButton(this.options.text.browse || i),
      (this.input.value = ''),
      (this.inputLabel.innerHTML = i),
      (this.imagePreview.style.backgroundImage = `url("${this.options.images.baseImage}")`),
...
var U = '/assets/custom-image.de30ffc0.svg';
const f = new k('myFirstImage'),
  J = document.querySelector('.upload-info-button--first');
J &&
  J.addEventListener('click', () => {
    console.log('First upload:', f, f.cachedFileArray);
  });
const v = new k('mySecondImage', {
    showDeleteButtonOnImages: !0,
    text: {
  ...
  w = document.querySelector('.upload-info-button--second');
w &&
  w.addEventListener('click', () => {
    console.log('Second upload:', v, v.cachedFileArray);
  });
window.addEventListener('fileUploadWithPreview:imagesAdded', (C) => {
  console.log('here'), console.log('e', C);
});
window.addEventListener('fileUploadWithPreview:imageDeleted', (C) => {});
window.addEventListener('fileUploadWithPreview:clearButtonClicked', (C) => {});
